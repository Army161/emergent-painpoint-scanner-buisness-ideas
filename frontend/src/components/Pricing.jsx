    ...
    <div style={{ minHeight: '100vh', background: '#030303' }}>
      ...
      <div style={{ maxWidth: 880, margin: '0 auto', padding: '48px 24px 80px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          ...
          <h1 style={{ margin: '0 0 12px', fontSize: 40, fontWeight: 800, color: 'var(--text)', fontFamily: 'Plus Jakarta Sans' }}>Find the plan that's right for you</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          ...
          <div className="card" style={{ padding: '32px' }}>
            <div style={{ marginBottom: 24 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--text)', fontFamily: 'Plus Jakarta Sans' }}>Free</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 6 }}>
                <span style={{ fontSize: 36, fontWeight: 800, color: 'var(--text)', fontFamily: 'Plus Jakarta Sans' }}>$0</span>
                <span style={{ fontSize: 14, color: 'var(--subtle)' }}>/forever</span>
              </div>
              <p style={{ margin: 0, fontSize: 14, color: 'var(--text-medium)' }}>Perfect for exploring what's possible</p>
            </div>
            ...
            {user ? (
              <div style={{ padding: '12px', borderRadius: 8, border: '1px solid var(--border)', textAlign: 'center', color: user.is_premium ? 'var(--subtle)' : 'var(--muted)', fontSize: 14 }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <div className="card" style={{ padding: '32px', position: 'relative', zIndex: 1, background: 'var(--surface)' }}>
              <div style={{ marginBottom: 24 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <Zap size={18} color="#818CF8" />
                  <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--text)', fontFamily: 'Plus Jakarta Sans' }}>Pro</span>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 6 }}>
                  <span style={{ fontSize: 36, fontWeight: 800, color: 'var(--text)', fontFamily: 'Plus Jakarta Sans' }}>$29</span>
                  <span style={{ fontSize: 14, color: 'var(--subtle)' }}>/month</span>
                </div>
                <p style={{ margin: 0, fontSize: 14, color: 'var(--text-medium)' }}>For founders who are serious about finding the right idea</p>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 28 }}>
                {FEATURES_PRO.map(f => (
                  <div key={f} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
              ...
              <button data-testid="upgrade-btn" onClick={handleUpgrade} disabled={loading || user?.is_premium}
                style={{ width: '100%', padding: '13px', borderRadius: 8, border: 'none', background: user?.is_premium ? 'var(--border)' : 'linear-gradient(135deg,#6366F1,#8B5CF6)', color: '#fff', fontSize: 15, fontWeight: 700, cursor: loading || user?.is_premium ? 'not-allowed' : 'pointer', transition: 'all 0.2s', boxShadow: user?.is_premium ? 'none' : '0 0 24px rgba(99,102,241,0.35)', fontFamily: 'Plus Jakarta Sans' }}
              >
