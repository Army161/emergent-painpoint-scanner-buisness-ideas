...
const TypewriterContent = ({ text }) => {
  ...
  return (
    <div style={{ fontFamily: 'Inter', lineHeight: 1.75, color: '#D4D4D8' }}>
      {lines.map((line, idx) => {
        if (line.startsWith('## ')) return <h2 key={idx} style={{ margin: '20px 0 8px', fontSize: 16, fontWeight: 700, color: '#FAFAFA', fontFamily: 'Plus Jakarta Sans', borderBottom: '1px solid #18181B', paddingBottom: 6 }}>{line.slice(3)}</h2>;
        if (line.startsWith('**') && line.endsWith('**')) return <p key={idx} style={{ margin: '8px 0 4px', fontWeight: 700, color: '#FAFAFA', fontSize: 14 }}>{line.slice(2, -2)}</p>;
...
const IdeaDetail = () => {
  ...
  if (!idea) return <div style={{ minHeight: '100vh', background: '#030303', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#71717A' }}>Idea not found</div>;
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#030303' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '28px 24px 60px' }}>
        ...
        {/* Breadcrumb */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24, fontSize: 13, color: '#52525B' }}>
          <Link to="/dashboard" style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#71717A', textDecoration: 'none' }}>
            <ArrowLeft size={14} />Discover
          </Link>
          <ChevronRight size={12} />
          <span style={{ color: '#A1A1AA' }}>{idea.title}</span>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 24, alignItems: 'start' }}>
          ...
          <div>
            ...
            <div className="card" style={{ padding: '28px 28px 24px', marginBottom: 20 }}>
              <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
                ...
                <span style={{ fontSize: 12, padding: '3px 10px', borderRadius: 99, border: '1px solid #27272A', color: '#A1A1AA' }}>{idea.category}</span>
              ...
              <h1 style={{ margin: '0 0 16px', fontSize: 26, fontWeight: 800, color: '#FAFAFA', lineHeight: 1.3, fontFamily: 'Plus Jakarta Sans' }}>{idea.title}</h1>
              <p style={{ margin: 0, fontSize: 15, color: '#A1A1AA', lineHeight: 1.7 }}>{idea.description}</p>
              ...
              {idea.pain_quote && (
                <blockquote style={{ margin: '20px 0 0', padding: '14px 18px', borderLeft: `3px solid ${painColor}`, background: `${painColor}08`, borderRadius: '0 8px 8px 0' }}>
                  <p style={{ margin: 0, fontSize: 13, color: '#D4D4D8', lineHeight: 1.7, fontStyle: 'italic' }}>{idea.pain_quote}</p>
                </blockquote>
              )}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
              {[
                ...
                <div key={label} className="card" style={{ padding: '20px 16px', textAlign: 'center' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, marginTop: 8, color: '#71717A', fontSize: 11 }}>
            ...
            <div className="card" style={{ padding: '20px 24px', marginBottom: 20 }}>
              <h3 style={{ margin: '0 0 10px', fontSize: 14, fontWeight: 700, color: '#FAFAFA', fontFamily: 'Plus Jakarta Sans' }}>Competition Gap Analysis</h3>
              <p style={{ margin: 0, fontSize: 14, color: '#A1A1AA', lineHeight: 1.7 }}>{idea.competition_analysis}</p>
            </div>

            {/* Tags */}
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 24 }}>
              {(idea.tags || []).map(tag => (
                <span key={tag} style={{ padding: '4px 12px', borderRadius: 99, border: '1px solid #27272A', color: '#71717A', fontSize: 12 }}>{tag}</span>
              ))}
            </div>
            ...
            {(brief || copy) && (
              <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                {/* Tabs */}
                <div style={{ display: 'flex', borderBottom: '1px solid #18181B', background: '#070707' }}>
                  {[
                    ...
                    <button key={key} data-testid={`tab-${key}`} onClick={() => setActiveTab(key)}
                      style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '14px 20px', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600,
                        ...
                        color: activeTab === key ? '#FAFAFA' : '#52525B',
                  ...
                  <button onClick={() => copyToClipboard(currentContent)} data-testid="copy-btn"
                    style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6, padding: '14px 20px', border: 'none', background: 'transparent', cursor: 'pointer', color: '#71717A', fontSize: 13 }}
          ...
          <div style={{ position: 'sticky', top: 80 }}>
            ...
            <div className="card" style={{ padding: '20px', marginBottom: 16 }}>
              <h3 style={{ margin: '0 0 16px', fontSize: 13, fontWeight: 700, color: '#A1A1AA', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Market Metrics</h3>
              {[
                { label: 'Total Addressable Market', value: idea.market_size, color: '#FAFAFA' },
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <button data-testid="generate-copy-btn" onClick={generateCopy} disabled={generating.copy}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '14px', borderRadius: 10, border: '1px solid #3F3F46', background: 'transparent', color: '#FAFAFA', fontSize: 14, fontWeight: 700, cursor: generating.copy ? 'not-allowed' : 'pointer', transition: 'all 0.2s', fontFamily: 'Plus Jakarta Sans' }}
              >
                <Zap size={16} />
                {generating.copy ? 'Generating...' : copy ? 'Regenerate Copy' : 'Generate Landing Copy'}
              </button>
              <button data-testid="save-idea-btn" onClick={handleSave}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '14px', borderRadius: 10, border: `1px solid ${saved ? 'rgba(99,102,241,0.4)' : '#27272A'}`, background: saved ? 'rgba(99,102,241,0.1)' : 'transparent', color: saved ? '#818CF8' : '#A1A1AA', fontSize: 14, fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s' }}
              >
            ...
            {!user?.is_premium && (
              <div style={{ marginTop: 16, padding: '16px', borderRadius: 10, background: 'linear-gradient(135deg, rgba(99,102,241,0.1), rgba(139,92,246,0.1))', border: '1px solid rgba(99,102,241,0.2)' }}>
                <p style={{ margin: '0 0 10px', fontSize: 13, fontWeight: 700, color: '#FAFAFA' }}>
                  {user?.free_briefs_used >= 1 ? 'Unlock Unlimited Access' : '1 free brief included'}
                </p>
                <p style={{ margin: '0 0 12px', fontSize: 12, color: '#A1A1AA', lineHeight: 1.6 }}>
