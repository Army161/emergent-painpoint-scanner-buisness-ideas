...
async def export_pdf(idea_id: str, token: str = None, user=Depends(get_user)):
    ...
    pdf.cell(0, 12, "IdeaRadar", ln=True)
    ...
    pdf.cell(0, 5, f"Generated by IdeaRadar  |  {datetime.now(timezone.utc).strftime('%B %d, %Y')}", ln=True, align="C")
    buf = io.BytesIO()
    pdf.output(buf)
    buf.seek(0)
    safe_title = "".join(c if c.isalnum() or c in " -_" else "" for c in title)[:50]
    return StreamingResponse(buf, media_type="application/pdf",
        headers={"Content-Disposition": f'attachment; filename="IdeaRadar_{safe_title}.pdf"'})
